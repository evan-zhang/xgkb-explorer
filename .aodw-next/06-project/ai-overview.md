...
- **后端**：无自建应用后端；浏览器端默认通过 `TokenApiClient` 使用 `access-token` 调玄关知识库服务，`OpenApiClient` 仅作为显式兼容模式；用户书架设置通过 document-database 的 `/document-database/xgkb/v1/settings` 云同步；`src/lib/auth.ts` 调 Cwork/钉钉登录相关接口。
- **数据库**：无本项目数据库；数据来自玄关知识库服务和 Cwork 认证服务。
- **消息系统 / 缓存**：无自建消息系统；浏览器端使用 React state + localStorage 保存认证、API 连接配置和 settings 缓存，书架入口/当前书架/预览偏好/收藏以服务端 settings 为跨浏览器权威数据。
...
2. 登录回调将 auth code 换成 Cwork `xgToken`，应用默认创建 `TokenApiClient`，并在知识库请求中发送 `access-token` 请求头。
3. 用户可在设置中保存 `apiMode`、serverUrl、previewMode 和一个或多个目录配置；只有显式选择 Open API 兼容模式时才需要 AppKey。
4. 登录成功后应用调用 settings API 拉取服务端书架配置；云端存在配置时覆盖本地缓存，云端不存在且本地有历史配置时经用户确认迁移到服务器。
5. `useApiClient` 创建统一 `KbApiClient` 契约下的 `TokenApiClient` 或 `OpenApiClient`。
6. 默认全部空间：`directoryId` 为空，`useProjectsHub` 调 `findAllProjects?nameKey=&bizCode=ordinary&appCode=kz_doc` 展示当前登录用户可见空间。
7. 自定义书架：`directoryId` 非空，`useProjectsHub` 直接调用 `getChildFiles(directoryId)`，展示该目录下的一级子文件夹作为项目。
8. 空间卡片以 `entryKind: "space"` 作为导航层，不显示收藏或分享；点击空间后调 `getLevel1Folders(projectId)` 展示可放入书架的一级目录。
9. 点击空间下的一级目录会把该目录 ID 保存为 `spaces` 书架入口，并切换到该书架；目录型书架仍以文件夹 ID 调 `getChildFiles(folderId)`。
10. 目录名称可通过 `batchGetMeta(directoryId)` 尝试自动读取。
11. 选择项目后进入双栏详情页：左侧目录树，右侧文件/文件夹列表，文件通过弹窗预览。
12. 预览根据 `previewMode` 选择 KB 预览服务或 `getDownloadInfo(fileId).downloadUrl`；自渲染预览会先 fetch 存储地址并打开 Blob URL，避免直接打开 `downloadUrl` 触发下载；文件点击直接复用新标签页打开策略，不再进入内置弹窗。

`projectId` 必须绑定当前 API client。token、AppKey、认证模式或 serverUrl 变化时必须丢弃旧 `projectId`，避免用旧用户/旧会话的项目 ID 查询新会话数据。

```text
[ Browser / React SPA ]
        |
        | localStorage: auth session/xgToken, apiMode, serverUrl, optional appKey, settings cache
        v
[ Auth Gate: DingTalkLogin ] ──> [ App Shell ] ──> [ ProjectsHub ] ──> [ ProjectDetail ]
        ...
        |                              v
        |                       [ KbApiClient contract ]
        |                              |
        v                              v
[ Cwork / DingTalk Login APIs ]  [ Xuan Guan Knowledge Base API: TokenApiClient default / OpenApiClient compatibility ]
```

Production traffic goes to `https://tpr.20100706.xyz/xgkb/`. API calls can use the upstream service root URL directly or the Caddy same-origin proxy `/xgkb-api/`; Open API compatibility mode normalizes the same setting back to `/open-api/`.
...
src/
  ...
  components/
    ...
    ConfigModal.tsx               # auth mode/server/preview mode/directory entry config UI
  ...
  lib/
    ...
    api.ts                        # KbApiClient contract, TokenApiClient, and OpenApiClient
    config.ts                     # localStorage config schema and migration
    preview.ts                    # file open / new-tab preview strategy
    hooks.ts                      # API client, project loading, tree/content hooks
    types.ts                      # knowledge-base API path constants and response types
...
### config
- Owns persisted localStorage schema: apiMode, serverUrl, optional appKey, previewMode, spaces, activeSpaceId, settingsRevision.
- Owns local cache helpers for starred project ids; server settings are synced through `src/lib/settings.ts`.
- Current `SpaceEntry` model: `{ id, name, directoryId }`; empty `directoryId` means the current user's visible project list.
- Owns UI for creating/editing directory entries.
- Key files: `src/lib/config.ts`, `src/components/ConfigModal.tsx`.

### api-client
- Wraps Xuan Guan knowledge-base calls and maps result envelopes into `ApiResult<T>`. Default auth uses `access-token`; Open API AppKey remains a compatibility mode.
- Key files: `src/lib/api.ts`, `src/lib/types.ts`.

### data-hooks
- Owns directoryId loading, visible project list loading, folder tree child loading, and README/index preview lookup.
...
- **API client 边界**：`projectId` 必须绑定当前 `KbApiClient`；token、AppKey、认证模式或 serverUrl 变化时必须丢弃旧 `projectId`。
- **配置兼容**：localStorage schema 变更必须迁移旧配置，不能让已有用户目录配置丢失。
- **书架云同步**：`spaces`、`activeSpaceId`、`previewMode`、`starredProjectIds` 登录后以 settings API 为跨浏览器权威数据；`apiMode`、`serverUrl`、`appKey` 仍只保存在本地。
- **书架加载语义**：空 `directoryId` 表示调用 `findAllProjects` 展示当前用户可见空间；非空 `directoryId` 表示直接以该目录 ID 调 `getChildFiles(directoryId)`。
- **旧配置迁移**：旧 `spaceId/path/rootFileId` 等字段必须尽量迁移为新的 `directoryId` 模型或保持可读。
- **预览一致性**：文件点击和“新标签页打开”应使用同一种方案；不能直接打开存储 `downloadUrl` 导致浏览器下载，需优先转 Blob URL 预览。
...
- 2026-06-24：主线引入 `TokenApiClient`，登录后默认使用 `access-token` 访问知识库，Open API AppKey 降级为兼容模式。
- 2026-06-24：默认入口从个人空间根目录调整为 `findAllProjects` 返回的当前用户可见空间列表；空间卡片进入后展示可放入书架的一级目录。
- 2026-06-30：接入 document-database settings API，一期同步个人书架、当前书架、预览偏好和收藏项目。
