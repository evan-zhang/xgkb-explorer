...
- **后端**：无自建应用后端；浏览器端默认通过 `TokenApiClient` 使用 `access-token` 调玄关知识库服务，`OpenApiClient` 仅作为显式兼容模式；`src/lib/auth.ts` 调 Cwork/钉钉登录相关接口。
...
4. `useApiClient` 创建统一 `KbApiClient` 契约下的 `TokenApiClient` 或 `OpenApiClient`。
5. 默认全部空间：`directoryId` 为空，`useProjectsHub` 调 `findAllProjects?nameKey=&bizCode=ordinary&appCode=kz_doc` 展示当前登录用户可见空间。
6. 自定义书架：`directoryId` 非空，`useProjectsHub` 直接调用 `getChildFiles(directoryId)`，展示该目录下的一级子文件夹作为项目。
7. 空间卡片以 `entryKind: "space"` 作为导航层，不显示收藏或分享；点击空间后调 `getLevel1Folders(projectId)` 展示可放入书架的一级目录。
8. 点击空间下的一级目录会把该目录 ID 保存为 `spaces` 书架入口，并切换到该书架；目录型书架仍以文件夹 ID 调 `getChildFiles(folderId)`。
9. 目录名称可通过 `batchGetMeta(directoryId)` 尝试自动读取。
10. 选择项目后进入双栏详情页：左侧目录树，右侧文件/文件夹列表，文件通过弹窗预览。
11. 预览根据 `previewMode` 选择 KB 预览服务或 `getDownloadInfo(fileId).downloadUrl`；自渲染预览会先 fetch 存储地址并打开 Blob URL，避免直接打开 `downloadUrl` 触发下载；文件点击直接复用新标签页打开策略，不再进入内置弹窗。
...
[ Browser / React SPA ]
        |
        | localStorage: auth session/xgToken, apiMode, serverUrl, optional appKey, previewMode, spaces[{directoryId}]
        v
...
### config
- Owns persisted localStorage schema: apiMode, serverUrl, optional appKey, previewMode, spaces, activeSpaceId.
